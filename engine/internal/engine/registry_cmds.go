...
// DefaultRegistry is the official FreeRein registry: a static
// sha-pinned index (spec/registry.md), generated by rein-publish into
// this repo's committed registry/ directory and served by GitHub
// Pages. Selection order: --registry flag > registry: in harness.yaml
// > this default.
const DefaultRegistry = "https://joaofreitas04.github.io/freerein/index.json"

// registrySource resolves which registry a command talks to. It never
// returns "": with a shipped default, "no registry configured" is not
// a state that exists anymore (v0.1's NO_REGISTRY is retired).
func registrySource(cfg *Config, override string) string {
	if override != "" {
		return override
	}
	if cfg.Registry != "" {
...
func (g *Engine) openRegistry(e *envelope.Envelope, cfg *Config, override string) *registry.Index {
	src := registrySource(cfg, override)
